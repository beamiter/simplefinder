...
- `BINARY`：找到的是哪个 daemon、它是不是比旁边的 Rust 源码还旧、它的版本对不对得上
  ...
  真正卡死的二进制也照样要给出结论——`job_start()` 对着它会成功返回、exit 回调永远
  不来——所以探测带 3 秒期限：到点停掉子进程，这行改写成 `[ERROR] version: the
  binary did not answer --version within 3s`，同样就地重画。之后再跑一次
  `:SimpleFinderHealth` 会重试一次，一时卡顿不至于让这条红线留一整个会话
...
所有运行时读取都检查类型；类型写错会被报告，并安全回退到默认值，不会从 redraw 或 timer
callback 里抛出 Vim9 类型异常。拼错的选项则是最没有症状的那种故障：名字永远读不到，
插件用默认值继续跑，而你写的那一行什么都不做。所以插件把选项全集声明在一处，任何其它
`g:simplefinder_` 名字都按拼写错误报出来（`g:loaded_simplefinder` 和裸的
`g:simplefinder` 不算）。
...
Vim 与后端通过标准输入/输出上的一行一个 JSON 消息通信。单条请求限制为 8 MiB，
超限记录会被拒绝且读取从下一行恢复；stdin 结束后，已经接受的查询及排队中的回复
仍会完整写出。每次查询都有独立 ID；新查询会取消旧任务，过期结果不会写入当前面板。
